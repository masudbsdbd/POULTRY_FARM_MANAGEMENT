@extends('layouts.vertical', ['title' => 'Datatables'])

@section('css')
@vite(['node_modules/datatables.net-bs5/css/dataTables.bootstrap5.min.css', 'node_modules/datatables.net-responsive-bs5/css/responsive.bootstrap5.min.css', 'node_modules/datatables.net-buttons-bs5/css/buttons.bootstrap5.min.css', 'node_modules/datatables.net-select-bs5/css/select.bootstrap5.min.css', 'node_modules/mohithg-switchery/dist/switchery.min.css'])
@endsection

@section('content')
<!-- Start Content-->
<div class="container-fluid">

    @include('layouts.shared.page-title', ['title' => $pageTitle, 'subtitle' => $pageTitle])

    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        <h4 class="header-title">{{ $pageTitle }}</h4>
                        @can('damage-create')
                        <a href="{{ route('damage.create') }}" class="mb-2 btn btn-primary waves-effect waves-light">Add
                            New</a>
                        @endcan
                    </div>

                    <table id="basic-datatable" class="table dt-responsive nowrap w-100">
                        <thead>
                            <tr>
                                <th>SL</th>
                                <th class="text-center">Supplier</th>
                                <th class="text-center">Product</th>
                                <th class="text-center">Damage Qty</th>
                                <th class="text-center">Total Damage Price</th>
                                <th class="text-center">Damage Date</th>
                                <th class="text-center">Damage Status</th>
                                <!-- <th class="text-center">Supplier Status</th> -->
                                <th class="text-end">Action</th>
                            </tr>
                        </thead>


                        <tbody>
                            @foreach ($damages as $item)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td class="text-center">{{ @$item->supplier->name }}</td>
                                <td class="text-center">
                                    {{ $item->product->name }}
                                </td>
                                <td class="text-center">{{ $item->qty }} units</td>
                                <td class="text-center">{{ showAmount($item->total_damage_price) }} Tk</td>
                                <td class="text-center">{{ showDateTime($item->entry_date, true) }}</td>
                                <td class="text-center">
                                    @if ($item->damage_status == 0)
                                    <span class="badge badge-soft-info">On Going</span>
                                    @else
                                    <span class="badge badge-soft-danger">Damaged</span>
                                    @endif
                                </td>
                                <!-- <td class="text-center">
                                    @if ($item->status == 0)
                                    <span class="badge badge-soft-dark">Pending</span>
                                    @elseif($item->status == 1)
                                    <span class="badge badge-soft-primary">Replacement</span>
                                    @elseif($item->status == 2)
                                    <span class="badge badge-soft-info">Repaire</span>
                                    @else
                                    <span class="badge badge-soft-danger">Decline</span>
                                    @endif
                                </td> -->
                                <td class="text-end">
                                    @can('damage-edit')
                                    <a href="{{ route('damage.edit', $item->id) }}"
                                        class="btn btn-primary waves-effect waves-light">
                                        <i class="mdi mdi-grease-pencil"></i></a>
                                    @endcan
                                    @can('damage-delete')
                                    <button type="button"
                                        class="btn btn-danger waves-effect waves-light confirmationBtn"
                                        data-question="@lang('Are you sure to delete this sell?')"
                                        data-action="{{ route('damage.delete', $item->id) }}"><i
                                            class="mdi mdi-trash-can-outline"></i></button>
                                    @endcan

                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>

                </div> <!-- end card body-->
            </div> <!-- end card -->
        </div><!-- end col-->
    </div>
    <!-- end row-->
</div> <!-- container -->


<x-confirmation-modal></x-confirmation-modal>
@endsection

@section('script')
@vite(['resources/js/app.js', 'resources/js/pages/datatables.init.js', 'resources/js/pages/form-advanced.init.js'])
@endsection