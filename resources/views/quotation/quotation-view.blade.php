        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-6">
                            <h4 class="header-title">Quotation Name: <span id="quotationName">{{ $quotation->title }}</span></h4>
                    <h4 class="header-title">Quotation Number: <span id="quotationNumber">{{ $quotation->quotation_number }}</span></h4>
                        </div>
                        <div class="col-6 text-end">
                            <a target="_blank" href="{{ route('quotation.pdf', $quotation->id) }}" class="btn btn-sm btn-info mb-2">Print Quotation</a>
                        </div>
                    </div>
                                ...
                                @foreach ($quotation->items as $item)
                                    <tr class="text-center">
                                        ...
                                        <td class="d-flex flex-column align-items-center">
                                            @php
                                                $usedPercent = round(($item->used_qty / $item->qty) * 100);
                                            @endphp
                                                <a href="{{ route('challan.challanItems', [$item->quotation_id, $item->product_id]) }}" class="btn btn-sm btn-info mb-2">{{ $item->used_qty }}</a>
                                        ...
                                        <td>
                                            @php
                                                $approvedPercent = round(($item->approved_qty / $item->qty) * 100);
                                            @endphp
                                            <div  class="d-flex flex-column align-items-center">
                                                <a href="{{ route('approval.approvedItems', [$item->quotation_id, $item->product_id]) }}" class="btn btn-sm btn-primary mb-2">{{ $item->approved_qty }}</a>
                                                {{-- {{ $item->approved_qty }} --}}
                                                <div class="progress" style="height:20px; width:120px; position: relative;">
                                                    <div class="progress-bar bg-success" role="progressbar" style="width: {{ $approvedPercent }}%;">
                                                    {{ $approvedPercent }}%
                                                    </div>
                                                    @if ($approvedPercent < 15 )
                                                        <p style="text-align: center; position: absolute; left: 40%; font-size: 12px;">{{ $approvedPercent }}%</p>
                                                    @endif
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            @php
                                                $invoicedPercent = round(($item->invoiced_qty / $item->qty) * 100);
                                            @endphp
                                            <div  class="d-flex flex-column align-items-center">
                                                <a href="{{ route('invoice.invoicedItems', [$item->quotation_id, $item->product_id]) }}" class="btn btn-sm btn-primary mb-2">{{ $item->invoiced_qty }}</a>
                                                <div class="progress" style="height:20px; width:120px; position: relative;">
