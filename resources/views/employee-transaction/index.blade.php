                        ...
                        <table id="basic-datatables" class="table dt-responsive nowrap w-100">
                            ...
                            <tbody>
                                @foreach ($transactions as $item)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            {{-- @if ($item->advance > 0)
                                                <span class="text-success fw-bold">Advance: {{ $item->advance }} Tk</span>
                                            @elseif($item->due > 0)
                                                <span class="text-danger fw-bold">Due: {{ $item->due }} Tk</span>
                                            @else
                                                <span class="text-muted fw-bold">Settled</span>
                                            @endif --}}
                                            @php
                                                $advanceDue = $item->total_advance - $item->total_due;
                                            @endphp

                                            @if ($advanceDue > 0)
                                                Advance: {{ $advanceDue }} Tk
                                            @elseif ($advanceDue < 0)
                                                Due: {{ abs($advanceDue) }} Tk
                                            @else
                                                <span class="text-muted fw-bold">Settled</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="text-end">
                                            <a title="Employee Transaction History"
                                                href="{{ route('employee.transaction.monthly.details', $item->employee->id) }}"
                                ...
                                <tr>
                                    ...
                                    <td></td>
                                    <td></td>
                                    <td class="text-center">{{ showAmount(getEmpTrSum('total_paid', 0, true)) }} Tk</td>
                                    <td class="text-center">{{ showAmount(getEmpTrSum('punishment', 0, true)) }} Tk</td>
                                    <td></td>
                                    <td></td>
                                </tr>
                            </tbody>
