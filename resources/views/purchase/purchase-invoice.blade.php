...
<body>
    <div class="invoice-container">
        ...
        <table class="header">
            <tr>
                ...
                <td class="details">
                    ...
                    <p>Created: {{ showDateTime($purchase->created_at) }}</p>
                </td>
            </tr>
        </table>
        ...
        {{--<table class="payment-table">
            ...
            <tbody>
                <tr>
                    <td>{{ $purchase->account->payment_method == 1 ? 'Cash' : 'Bank' }}</td>
                    <td>{{ showAmount($purchase->total_price) }} Tk</td>
                    <td>{{ showAmount($purchase->due_to_company) }} Tk</td>
                </tr>
            </tbody>
        </table>

        <!-- Payment Details -->
        @if ($purchase->account->payment_method == 2)
            <table class="bank-table">
                ...
                <tbody>
                    <tr>
                        <td>{{ $purchase->account->bankTransaction->bank->bank_name }}</td>
                        <td>{{ $purchase->account->bankTransaction->check_no }}</td>
                        <td>{{ $purchase->account->bankTransaction->withdrawer_name }}</td>
                    </tr>
                </tbody>
            </table>
        @endif--}}
        ...
        <table class="product-details-table">
            ...
            <tbody>
                @foreach ($purchase->items as $item)
                    <tr>
                        ...
                        <td>{{ $item->warehouse->warehouse_name ?? "N/A" }}</td>
                        <td>{{ showAmount($item->price) }} Tk</td>
            ...
            <tfoot>
                <tr>
                    ...
                    <td>{{ showAmount($purchase->main_price) }} Tk</td>
                </tr>
            </tfoot>
        </table>
        ...
        <div class="payment-summary" style="text-align: right;">
            <div class="summary-line">
                ...
                <span class="summary-value">{{ showAmount($purchase->main_price) }} Tk</span>
            </div>
            <div class="summary-line">
                <span class="summary-label">Commission:</span>
                <span class="summary-value">{{ showAmount($purchase->commission) ?? 0 }} %</span>
            </div>
            <div class="summary-line">
                <span class="summary-label">Discount:</span>
                <span class="summary-value">{{ showAmount($purchase->discount) ?? 0 }} Tk</span>
            </div>
            <div class="summary-line total">
                <span class="summary-label">Total Payable:</span>
                <span class="summary-value">{{ showAmount($purchase->total_price) }} Tk</span>
            </div>
            <div class="summary-line">
                <span class="summary-label">Due:</span>
                <span class="summary-value">{{ showAmount($purchase->due_to_company) }} Tk</span>
            </div>
            <hr>
            <div class="summary-line total">
                <span class="summary-label">Total Paid:</span>
                <span class="summary-value">{{ showAmount($purchase->payment_received) }} Tk</span>
            </div>
        </div>
