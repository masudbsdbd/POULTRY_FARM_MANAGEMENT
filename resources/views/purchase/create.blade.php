                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            ...
                            <h4>{{ isset($purchase) ? 'Batch Code:' . $purchase->batch->batch_code : '' }}</h4>
                        </div>
                        <form
                            ...
                            <div class="row">
                                    ...
                                    <select id="select-suppliers" class="form-control" data-toggle="select2"
                                        ...
                                        @foreach ($suppliers as $item)
                                            <option value="{{ $item->id }}" data-item="{{ $item }}"
                                                @selected(isset($purchase) ? $purchase->supplier_id == $item->id : false)>
                                                {{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <p id="supplierAdvanceParagraph"
                                        class="mt-1 {{ isset($purchase) ? ($purchase->supplier->advance > 0 ? '' : 'd-none') : 'd-none' }}">
                                        Current Advance Balance : <span
                                            id="supplierAdvance">{{ isset($purchase) ? showAmount($purchase->supplier->advance) . ' Tk' : 0 }}</span>
                                    </p>
                                </div> <!-- end col -->
                                                ...
                                                @if (isset($purchase_items) && !$purchase_items->isEmpty())
                                                    @foreach ($purchase_items as $item)
                                                        ...
                                                        <tr class="tableRow">
                                                            ...
                                                            <td>
                                                                <select id="select-suppliers" class="form-control"  data-toggle="select2"
                                                                    data-width="100%" name="warehouse_id[]" required>
                                                                    <option value="">Select</option>
                                                                    @foreach ($warehouses as $warehouse)
                                                                        <option value="{{ $warehouse->id }}" data-item="{{ $warehouse }}"
                                                                            @selected(isset($warehouse->id) ? $warehouse->id == $item->warehouse_id : false)>
                                ...
                                <div class="text-end mt-3">
                                    <button type="submit"
                                        class="btn btn-primary waves-effect waves-light">{{ isset($purchase) ? 'Update Purchase' : 'Add New Purchase' }}</button>
                                </div>
                ...
                <tr class="tableRow">
                    ...
                    <td>
                        <div class="input-group input-group-merge">
                            <input type="number" class="form-control productPrice unitPriceCalculation"
                                placeholder="Unit Price" name="unitPrice[]" value="${parseFloat(productData.price).toFixed(2)}" autocomplete="off" required>
                            <div class="input-group-text">Tk / ${productData.unit.name}</div>
                        </div>
                    </td>
                    <td>
                        <input type="text" class="form-control totalPrice" name="priceTotal[]" placeholder="Total" readonly>
