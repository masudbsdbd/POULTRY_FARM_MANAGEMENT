...
class EmployeeTransactionController extends Controller
{
    ...
    public function payment(Request $request)
    {
        ...
        if ($payableData) {
            ...
            if($transaction->punishment > 0){
                $income = new Income();
                $income->entry_type = 1;
                $income->effective_amount = $transaction->punishment;
                $income->entry_by = auth()->user()->id;
                $income->debit_or_credit = 'credit';
                $income->description = "Employee salary income amount of " . showAmount($transaction->punishment) . " Tk as punishment. ";
                $income->save();
            }
        ...
        return to_route('employee.transaction.salary.trx')->withNotify($notify);
    }
