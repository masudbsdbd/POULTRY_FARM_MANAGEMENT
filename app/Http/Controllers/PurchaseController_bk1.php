...
class PurchaseController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $advanceAccountRow = Account::wherePurchaseId($purchase->id)->whereFromAdvance(1)->first();
    ...
    public function store(Request $request, $id = 0)
    {
        ...
        if ($id > 0) {
            ...
            if ($prviousParchasePaymentReceived > $request->calculatedPrice) {
                ...
                $advanceAccountRow = Account::wherePurchaseId($id)->whereFromAdvance(1)->whereIsAdvance(2)->first();
        ...
        if ($request->balance > $request->calculatedPrice) {
            ...
            $paidAdvanceDesc = "Paid advance payment of " . $calculatedDebit . " Tk to the supplier " . $findSupplier->name . ".";

            $this->supplierAccountUpdate($request, $findSupplier, $calculatedDebit, $purchase->id, $paidAdvanceDesc, 1);
        } else {
            ...
            if ($getAdvance > 0) {
                ...
                if ($remainingAdvance > 0) {
                    ...
                    $removeAccounts = Account::where('purchase_id', $id)->get();
                    // return $removeAccount;
                    foreach($removeAccounts as $item){
                        $item->delete();
                    }
                    ...
                    $inputtedAmount = $request->balance - $getAdvance;
                    $fromAdvance = false;
                    // $accDesc = " The payment was made using the advance amount of " . showAmount($getAdvance, 2, false) . " Tk from supplier." . " Previous Balance: " . showAmount($getAdvance, 2, false) . " Tk, Current Balance: " . 0 . " Tk";
                    $accDesc = " The total parchase payment was partially made using the advance amount of " . showAmount($getAdvance, 2, false) . " Tk from supplier." . " Previous Balance: " . showAmount($getAdvance, 2, false) . " Tk, Current Balance: " . 0 . " Tk";

                    $deductedAdvanceDesc = "Deduction of " . showAmount($getAdvance, 2, false) . " Tk from the advance amount of supplier " . $findSupplier->name . ".";
                    $this->supplierAccountUpdate($request, $findSupplier, $getAdvance, $purchase->id, $deductedAdvanceDesc, 2, true);
                }
            }
        }
        ...
        $accArr = [
            ...
            'description'       => "Purchase(" . $purchase->batch->batch_code  . ") payment of " . showAmount($inputtedAmount, 2, false) . " Tk has been successfully paid to Supplier " . $purchase->supplier->name . "." . ($due ? ' There was a due of ' . $due . ' Tk.' : '') . (isset($accDesc) ? $accDesc : ''),
            'supplier_id'       => $purchase->supplier_id,
            'payment_method'    => $request->payment_method,
        ];

        if ($fromAdvance == true) {
            if ($advanceDeduction == 2) {
                $accArr['is_advance'] = 2;
            }
            $accArr['from_advance'] = 1;
    ...
    public function supplierAccountUpdate($request, $supplier, $amount, $id, $desc, $isAdvance, $formAdvance = false)
    {
        $accArr = [
            'purchase_id'       => $id,
        ...
        $account = updateAcc($accArr, 'purchase_id', $id, $formAdvance, $isAdvance);
